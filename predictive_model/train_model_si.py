...
import fnet
import fnet.data
import fnet.fnet_model_si  # line 194
import json
import logging
import numpy as np
import os
...
def get_dataloader(remaining_iterations, opts, validation=False):
    ...
    ds_patch = fnet.data.BufferedPatchDataset2(
        dataset = ds,
...
def main():
    ...
    parser.add_argument('--out_drop_index', type=int, help='index of added input in the previous target list')
    ...
    #     # state_dict = fnet.functions.load_pretrained_model(path_model_state, opts.in_dim - 1, opts.out_dim + 1, 0, gpu_ids=opts.gpu_ids, id = 0) 
    #     logger.info('model loaded from: {:s}'.format(path_model_state))
    ...
    for i, (signal, target) in enumerate(dataloader_train, model.count_iter):
        ...
        if ((i + 1) % opts.interval_save == 0) or ((i + 1) == opts.n_iter):
            ...
            if opts.path_dataset_val_csv is not None:
                ...
                val_loss, val_mse, val_pcc = model.predict(val_images, opts.input_index, opts.target_index, opts.patch_size) # to_do: only compute validation loss
                # pred_val, corr_array, r2_array, val_loss = model.predict(val_images, opts.input_index, opts.target_index, opts.patch_size)  
                # pred_train, corr_train, _, train_loss = model.predict(train_images, opts.input_index, opts.target_index, opts.patch_size)
                ...
                # np.save(path_save_corr, corr_array) 
                ...
                # logger.info('loss val log saved to: {:s}'.format(path_losses_train_csv)) 
